...
func (z *ZBIClient) CreateInstance(ctx context.Context, project *model.Project, instance *model.Instance) error {
	...
	peers := GetPeerInstances(ctx, request.Peers)
	...
	_, err = z.client.ApplyResources(ctx, objects[0])
	if err != nil {
	...
	if peers != nil && len(objects) > 1 {
		for index, _ := range peers {
			_, err = z.client.ApplyResources(ctx, objects[index])
			if err != nil {
...
// 	//TODO - use Repository service to get instances
...
func (z *ZBIClient) DeleteInstance(ctx context.Context, project *model.Project, instance *model.Instance) error {
	...
	resources, newResources, err := vars.ManagerFactory.GetProjectDataManager(ctx).CreateDeleteResource(ctx, projIngress, project, instance)
...
func (z *ZBIClient) RepairInstance(ctx context.Context, project *model.Project, instance *model.Instance) error {
	...
	_, err = z.client.ApplyResources(ctx, objects)
	if err != nil {
...
func (z *ZBIClient) StartInstance(ctx context.Context, project *model.Project, instance *model.Instance) error {
	...
	// instance.AddResources(resources...)
	log.Infof("Created %d resources for instance %s in project %s", len(resources), instance.Name, project.Name)
	return nil
}
...
func (z *ZBIClient) CreateSnapshot(ctx context.Context, project *model.Project, instance *model.Instance) error {
	...
	// instance.AddResources(resources...)
	log.Infof("created %d snapshot resources for instance %s in project %s", len(resources), instance.Name, project.Name)
...
func (z *ZBIClient) CreateSnapshotSchedule(ctx context.Context, project *model.Project, instance *model.Instance, schedule model.SnapshotScheduleType) error {
	...
	// instance.AddResources(resources...)
	log.Infof("created %d snapshot schedule resources for instance %s in project %s", len(resources), instance.Name, project.Name)

	return nil
}

func GetPeerInstances(ctx context.Context, peers []string) []model.Instance {

	var instances []model.Instance

	repo := vars.RepositoryFactory.GetRepositoryService()
	for _, peer := range peers {
		instance, err := repo.GetInstance(ctx, peer)
		if err == nil {
			instances = append(instances, *instance)
		}
	}
